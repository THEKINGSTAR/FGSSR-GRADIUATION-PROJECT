...
def get_recommendations(skills):
    # Check that the user entered at least one valid skill
    valid_skills = set(programmers_df.columns)
    if not skills:
        raise ValueError("Please choose at least one skill.")
    invalid_skills = set(skills) - valid_skills
    if invalid_skills:
        warnings.warn(f"Invalid skills: {', '.join(invalid_skills)}", UserWarning)
